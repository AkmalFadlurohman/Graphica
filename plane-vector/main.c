...
int main() {
    ...
    VectorPath* segitiga = createVectorPathFromFile("segitiga.txt");
    if (segitiga == NULL) {
        printf("Failed to load segitiga\n");
        return 0;
    }
...
void fillVector(VectorPath* path, unsigned int fillColor, unsigned int boundaryColor, int offsetX, int offsetY) {
    ...
    if (checkIfPathIsClosed(path)) {
     //    printf("gambar:%d,%d,%d,%d\n", path->minX,path->minY,path->maxX,path->maxY);
